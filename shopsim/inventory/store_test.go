...
func (s *stubStore) Reserve(ctx context.Context, request reserveRequest) (bool, error) {
	s.calls++
...
func TestStorageHTTP(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"insufficient stock", &stubStore{err: errInsufficientStock}, 409},
		{"unknown sku", &stubStore{err: errUnknownSKU}, 404},
		{"contention exhausted", &stubStore{err: errContention}, 503},
	} {
		t.Run(tc.name, func(t *testing.T) {
			recorder := httptest.NewRecorder()
			handler(tc.store).ServeHTTP(recorder, httptest.NewRequest("POST", "/reserve", strings.NewReader(`{"order_id":"o1","sku":"s1","quantity":2}`)))
